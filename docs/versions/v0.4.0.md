...
After TelegramInputSanitizer (basic — Issue #12):
  1. Strip bot mention syntax
  2. Wrap in <untrusted_content source="telegram:user:{userId}">

After TelegramInputSanitizer (hardened — Issue #17):
...
| Idle sessions | `IdleTimeout` (default 15min) | SessionManager background timer | Session suspended |
...
|---|---|---|---|
| A | Documentation & Instructions | #1–#2 | 0 |
| B | Multi-Session Foundation | #3–#7 | ~108 |
| C | Telegram Security Layer | #8–#11 | ~65 |
| D | Telegram Core Features | #12–#17 | ~120 |
| E | Extended Features | #18–#20 | ~35 |
| F | Testing & Release | #21–#22 | ~50 |
...
Phase A (no dependencies):
  #1 (Copilot Instructions + AGENTS.md)
  #2 (Architecture Docs)

Phase B (Multi-Session):
  #3 (Core Abstractions)       ← #1
  #4 (CorrelationContext)       ← #1
  #5 (SessionFactory)           ← #3
  #6 (SessionManager)           ← #3, #5
  #7 (Console Refactor)         ← #6
...
Phase D (Telegram Features):
  #12 (CommandRouter + basic sanitizer) ← #11, #10
  #13 (ResponseStreamer)                ← #10
  #14 (Approval Flow)                   ← #12, #13
  #15 (Session Integration)             ← #6, #12
  #16 (Polling Service)                 ← #11, #12
  #17 (Harden Input Sanitization)       ← #12

Phase E (Extended):
  #18 (File Exchange)            ← #15
  #19 (Health Monitor)           ← #16
  #20 (Dual-Mode Host)           ← #7, #16

Phase F (Release):
  #21 (Adversarial Tests)        ← #6, #11
  #22 (Release Docs)             ← ALL
```

### Issue Summary Table

| # | Title | Phase | Depends On | Primary Project | Est. Tests |
|---|---|---|---|---|---|
| 1 | Copilot instructions, custom instructions, and AGENTS.md update | A | — | .github/, AGENTS.md | 0 |
| 2 | Architecture documentation (MULTI-SESSION.md, TELEGRAM.md) | A | — | docs/ | 0 |
| 3 | Core abstractions (ISessionFactory, ISessionManager, ManagedSession, etc.) | B | 1 | Krutaka.Core | ~30 |
| 4 | CorrelationContext agent identity fields | B | 1 | Krutaka.Core | ~8 |
| 5 | SessionFactory implementation | B | 3 | Krutaka.Tools | ~20 |
| 6 | SessionManager with lifecycle management | B | 3, 5 | Krutaka.Core | ~35 |
| 7 | Console Program.cs refactor to ISessionManager | B | 6 | Krutaka.Console | ~15 |
| 8 | TelegramSecurityConfig and validation | C | 1 | Krutaka.Core | ~20 |
| 9 | Telegram audit events (default interface methods) | C | 4 | Krutaka.Core | ~15 |
| 10 | Krutaka.Telegram project scaffold | C | 3, 8 | Krutaka.Telegram | ~5 |
| 11 | ITelegramAuthGuard | C | 8, 10 | Krutaka.Telegram | ~30 |
| 12 | ITelegramCommandRouter + basic input sanitizer | D | 11, 10 | Krutaka.Telegram | ~25 |
| 13 | ITelegramResponseStreamer | D | 10 | Krutaka.Telegram | ~20 |
| 14 | Inline keyboard approval flow + HMAC | D | 12, 13 | Krutaka.Telegram | ~25 |
| 15 | Telegram ↔ ISessionManager integration | D | 6, 12 | Krutaka.Telegram | ~15 |
| 16 | Dual-mode polling service | D | 11, 12 | Krutaka.Telegram | ~20 |
| 17 | Harden input sanitization | D | 12 | Krutaka.Telegram | ~15 |
| 18 | File exchange via Telegram | E | 15 | Krutaka.Telegram | ~15 |
| 19 | Health monitoring and push notifications | E | 16 | Krutaka.Telegram | ~10 |
| 20 | Dual-mode host | E | 7, 16 | Krutaka.Console | ~10 |
| 21 | Adversarial tests (isolation + auth + callback + exhaustion) | F | 6, 11 | Tests | ~50 |
| 22 | Release documentation and verification | F | ALL | Docs | 0 |
...
5. **Issue #7 rollback**: If the Console Program.cs refactor causes >5 test failures, revert and file investigation issue (see Issue #7 rollback plan)
...
|---|---|---|---|
| Console regression from DI refactor | Medium | High | Issue #7 verifies all 1,290 tests pass, has explicit rollback plan |
...
| Sanitization rework between Issue #12 and #17 | Medium | Low | Basic sanitizer in #12, hardening in #17 — clean separation |
...
- `docs/architecture/MULTI-SESSION.md` — Multi-session isolation architecture (created in Issue #2)
- `docs/architecture/TELEGRAM.md` — Telegram security architecture (created in Issue #2)
- `docs/architecture/OVERVIEW.md` — Component architecture (updated in Issue #2)
- `docs/architecture/SECURITY.md` — Security model (updated in Issue #2)
...
- `docs/guides/TELEGRAM-SETUP.md` — Telegram setup guide (created in Issue #22)
- `CHANGELOG.md` — Version history
- `AGENTS.md` — Agent instructions (updated in Issue #1, verified in Issue #22)
