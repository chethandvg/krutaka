# Krutaka v0.4.0 — Telegram Integration & Multi-Session Architecture

> **Version:** v0.4.0
> **Status:** 🟡 In Progress
> **Predecessor:** v0.3.0 (✅ Complete — 1,289 tests passing, 1 skipped, graduated command execution)
> **Milestone:** v0.4.0 : Telegram Integration & Multi-Session Architecture (create on GitHub before issue creation)
...
The security model follows the project's established **ratchet principle**: new capabilities are added with proportional security controls. Every relaxation is gated. The security boundary bar from v0.1.0–v0.3.0 never moves down.
...
```csharp
// Program.cs — everything is singleton
...
1. **No remote access** — The agent can only be used via the local console terminal
2. **No concurrent users** — One session at a time, one user at a time
3. **Mobile inaccessible** — Cannot interact with the agent from a phone or tablet
4. **No unattended operation** — Agent stops when the console window closes

### Root Cause

Every mutable, session-specific component is registered as `AddSingleton`. Two concurrent operations would share:
- `_conversationHistory` in `AgentOrchestrator` — message interleaving
- `SessionId` and `TurnId` in `CorrelationContext` — audit log corruption
- Directory grants in `ISessionAccessStore` — security violation (User A's grants apply to User B)
- Command approvals in `ICommandApprovalCache` — security violation
- `_approvalCache` (internal `ConcurrentDictionary`) in `AgentOrchestrator` — tool "always approve" leaks across users

### Why Telegram First

Telegram provides the best security-to-effort ratio for a first remote interface:
- **Built-in authentication** — Telegram user IDs are stable and unforgeable by end users
- **No server hosting required** — Long polling works from behind NAT/firewalls
- **Structured interactions** — Inline keyboards map cleanly to approval flows
- **Rate limiting built-in** — Telegram's API has its own rate limits that complement ours
- **Encrypted transport** — All Telegram API calls are HTTPS
...
| G2 | Session lifecycle management | Create, idle detect, suspend, resume, terminate — all tested |
...
| G5 | Telegram security layer | Allowlist, rate limiting, lockout, HMAC callbacks — all adversarial tested |
| G6 | Remote approval flow | Inline keyboard buttons for all 3 approval types |
| G7 | Streaming responses | AgentEvent → Telegram with rate limit compliance (≤30 edits/min) |
...
| NG4 | Changes to existing tool implementations | v0.3.0 tools consumed as-is | — |
...
│    │                    │                │   │
│    ▼                    ▼                ▼   │
│ IToolRegistry   CorrelationContext  SessionStore │
...
│                     Session Manager                         │
│                                                             │
│  Session A (Telegram DM)     Session B (Telegram Group)    │
│  ┌────────────────────┐     ┌────────────────────┐        │
│  │ Orchestrator A      │     │ Orchestrator B      │        │
│  │ CorrelationCtx A    │     │ CorrelationCtx B    │        │
│  │ SessionStore A.jsonl│     │ SessionStore B.jsonl│        │
│  │ AccessGrants A      │     │ AccessGrants B      │        │
│  │ ApprovalCache A     │     │ ApprovalCache B     │        │
│  │ ToolRegistry A      │     │ ToolRegistry B      │        │
│  └────────────────────┘     └────────────────────┘        │
│                                                             │
│  Session C (Console)          Shared (stateless):          │
│  ┌────────────────────┐      • IClaudeClient               │
│  │ Orchestrator C      │      • ISecurityPolicy             │
│  │ CorrelationCtx C    │      • IAuditLogger                │
│  │ SessionStore C.jsonl│      • IAccessPolicyEngine         │
│  │ AccessGrants C      │      • ICommandRiskClassifier      │
│  │ ApprovalCache C     │      • ToolOptions (immutable)     │
│  │ ToolRegistry C      │                                    │
│  └────────────────────┘                                    │
└────────────────────────────────────────────────────────────┘
```
...
│ State        │              │                  │
│ Budget       │              │ ConcurrentDict   │
│ Orchestrator │              │ <Guid, Session>  │
│ CorrelationCtx│             │                  │
...
1. `_approvalCache` — internal `ConcurrentDictionary<string, bool>` for per-tool "always approve" — automatically per-session because it's created in the constructor
2. `_commandApprovalCache` — DI-injected `ICommandApprovalCache` for command-signature-level approvals (v0.3.0) — must be created per-session by the factory
...
```text
  Created ──► Active ──► Idle (no messages for IdleTimeout)
                │              │
                │              ▼
                │        Idle (grace period: 2× IdleTimeout)
                │              │
                │              ▼
                │        Suspended
                ...
                │              │   → SessionFactory.Create(request, originalGuid)
                │              │   → SessionStore.ReconstructMessagesAsync()
                │              │   → orchestrator.RestoreConversationHistory()
                │              │   (history reconstruction is caller's responsibility)
                │              │
...
|---|---|---|---|
| `IClaudeClient` | ✅ | | Stateless HTTP client, thread-safe |
| `ISecurityPolicy` | ✅ | | Stateless validation rules, immutable |
| `IAuditLogger` | ✅ | | Thread-safe Serilog, events tagged with SessionId |
| `IAccessPolicyEngine` | ✅ | | Stateless policy evaluation (deny-list, ceiling) |
| `ICommandRiskClassifier` | ✅ | | Stateless tier classification rules |
| `ToolOptions` | ✅ | | Immutable configuration loaded at startup |
| `AgentOrchestrator` | | ✅ | `_conversationHistory`, `_turnLock`, `_approvalCache` — per-session mutable state |
| `CorrelationContext` | | ✅ | Mutable SessionId, TurnId, RequestId, AgentId |
| `SessionStore` | | ✅ | Writes to session-specific JSONL file |
| `ISessionAccessStore` | | ✅ | Directory grants scoped to session |
| `ICommandApprovalCache` | | ✅ | Command approvals scoped to session |
| `ContextCompactor` | | ✅ | References per-session CorrelationContext |
| `IToolRegistry` | | ✅ | Tools scoped to session's working directory |

### Telegram Architecture

```text
┌─────────────┐     ┌─────────────────────────┐     ┌──────────────────┐
│  Telegram    │────►│   Krutaka.Telegram       │────►│  Session Manager │
│  Bot API     │◄────│   (new project)          │◄────│  → ManagedSession│
└─────────────┘     └─────────────────────────┘     │  → Orchestrator  │
                           │                         └──────────────────┘
                    ┌──────┴──────────────┐
                    │ Security Pipeline:  │
                    │  1. AuthGuard       │  ← allowlist, rate limit, lockout
                    │  2. CommandRouter   │  ← parse, sanitize, gate admin
                    │  3. SessionBridge   │  ← chat → session mapping
                    │  4. ResponseStreamer│  ← AgentEvent → messages
                    │  5. ApprovalHandler │  ← inline keyboards + HMAC
                    │  6. InputSanitizer  │  ← untrusted_content wrapping
                    │  7. FileHandler     │  ← upload/download validation
                    │  8. HealthMonitor   │  ← alerts, budget warnings
                    │  9. AuditLogger    │  ← every interaction logged
                    └─────────────────────┘
```
...
| **Latency** | 0–30s polling interval | Near real-time push |
...
Security verification on callback:
  1. HMAC-SHA256 verify → reject if tampered
  2. user_id == callback.From.Id → reject cross-user
  3. timestamp within approvalTimeout → reject expired
  4. nonce not previously used → reject replay
  5. All pass → route to orchestrator method
```
...
ToolCallCompleted ────────────►  Edit message: "✅ `git status` complete"
...
Rate limit: ≤30 edits per minute per chat (Telegram API limit)
```

### File Exchange

```text
Upload (User → Bot → Agent):
  1. User sends document via Telegram
  2. Validate extension (allowlist: .cs, .json, .md, .txt, ...)
  3. Block executables (.exe, .dll, .ps1, .bat, ...)
  4. Check double-extension bypass (file.txt.exe → blocked)
  5. Validate size (≤10MB configurable)
...
Events → Notifications:
  Startup              →  "🟢 Krutaka bot is online"       → Admin users
  Shutdown             →  "🔴 Krutaka bot shutting down"    → Admin users
  SecurityIncident     →  "⚠️ Security incident: {summary}" → Admin users
  Budget at 80%        →  "💰 Budget warning: 80% used"     → Specific chat
  Task completion      →  "✅ Task completed: {summary}"    → Specific chat
  Heartbeat (optional) →  "🟢 Krutaka is running"           → Admin users
...
"Console" (default — no Telegram config required):
  ┌──────────┐
  │ ConsoleUI │  ← existing behavior, single session via ISessionManager
  └──────────���

"Telegram" (requires valid Telegram config):
  ┌───────────────────┐
  │ TelegramBotService│  ← BackgroundService, headless, multi-session
  └───────────────────┘

"Both" (requires valid Telegram config):
  ┌──────────┐  ┌───────────────────┐
  │ ConsoleUI │  │ TelegramBotService│  ← concurrent, shared ISessionManager
  └──────────┘  └───────────────────┘
```
...
2. TelegramBotService receives Update
   ├── Kill switch check (if /killswitch in batch → process first)
   └── Normal processing:
3.     ITelegramAuthGuard.ValidateAsync(update)
       ├── FAIL: silent drop, log TelegramAuthEvent
       └── PASS: continue
4.     ITelegramCommandRouter.RouteAsync(update, authResult)
       ├── Parse command + arguments
       ├── Admin gating check
       ├── TelegramInputSanitizer.SanitizeMessageText() → wrap in <untrusted_content>
       └── Return CommandRouteResult
5.     ITelegramSessionBridge.GetOrCreateSessionAsync(chatId, userId, chatType)
       └── ISessionManager.GetOrCreateByKeyAsync(externalKey, request)
...
7.     ITelegramResponseStreamer.StreamResponseAsync(chatId, events, onInteractiveEvent)
       ├── TextDelta → buffer + edit message
       ├── ToolCallStarted/Completed/Failed → status messages
       ├── Approval events → onInteractiveEvent callback
       │   └── ITelegramApprovalHandler.SendApprovalRequestAsync()
       │       └── User presses inline button → HMAC verify → orchestrator.Approve/Deny
       └── FinalResponse → formatted MarkdownV2 message
...
├── Local console input (trusted OS user)
├── File system (sandboxed by IAccessPolicyEngine)
├── Child processes (sandboxed by Job Objects)
└── Claude API (outbound HTTPS only)

v0.4.0 Additions:
├── Telegram Bot API (inbound network traffic)      ← NEW
├── Multiple concurrent sessions                      ← NEW
├── Inline keyboard callback handling                 ← NEW
├── File upload/download via Telegram                 ← NEW
├── Health monitoring push notifications              ← NEW
└── All v0.3.0 surfaces remain unchanged
```
...
Update received
  ...
  │   │        return AuthResult.Invalid (NO REPLY SENT)
  │   └── YES → continue
  │
  ├── Is user locked out?
...
| Exponential backoff | 5s → 10s → 20s → ... → 120s cap on connection failure | Retry storm DoS |
...
Verification (when button pressed):
  ...
  3. if (receivedHmac != expectedHmac) → REJECT "callback tampering"
  4. if (callback.From.Id != payload.user_id) → REJECT "cross-user"
  5. if (now - payload.timestamp > approvalTimeout) → REJECT "expired"
  6. if (payload.nonce in usedNonces) → REJECT "replay"
...
After TelegramInputSanitizer (basic — Issue #139):
  1. Strip bot mention syntax
  2. Wrap in <untrusted_content source="telegram:user:{userId}">

After TelegramInputSanitizer (hardened — Issue #144):
...
| Idle sessions | `IdleTimeout` (default 15min) | SessionManager background timer | Session suspended (after 2× IdleTimeout grace period) |
...
| Double-extension check | Bypass via double extension | `file.txt.exe` → `.exe` blocked |
| Size limit (10MB) | Resource exhaustion / disk filling | 500MB file → rejected |
| Path traversal check | Escaping session temp directory | `../../etc/passwd` → rejected |
| Reserved device names | Windows device name exploit | `CON.txt`, `NUL.cs` → rejected |
...
**Critical:** `BotToken` is NOT in this config. It is stored in Windows Credential Manager via `ISecretsProvider` or in the `KRUTAKA_TELEGRAM_BOT_TOKEN` environment variable. There is no `BotToken` property on `TelegramSecurityConfig` — this is enforced by design.
...
|---|---|---|---|
| A | Documentation & Instructions | #128–#129 | 0 |
| B | Multi-Session Foundation | #130–#134, #156 | ~108 |
| C | Telegram Security Layer | #135–#138 | ~65 |
| D | Telegram Core Features | #139–#144 | ~120 |
| E | Extended Features | #145–#147 | ~35 |
| F | Testing & Release | #148–#149 | ~50 |
...
Phase B (Multi-Session):
  #130 (Core Abstractions)       ← #128
  #131 (CorrelationContext)       ← #128
  #132 (SessionFactory)           ← #130
  #156 (Session ID Preservation)  ← #130, #132
  #133 (SessionManager)           ← #130, #132, #156
  #134 (Console Refactor)         ← #133, #156

Phase C (Telegram Security):
  #135 (SecurityConfig)           ← #128
  #136 (Audit Events)             ← #131
  #137 (Scaffold)                 ← #130, #135
  #138 (AuthGuard)                ← #135, #137

Phase D (Telegram Features):
  #139 (CommandRouter + basic sanitizer) ← #138, #137
  #140 (ResponseStreamer)                ← #137
  #141 (Approval Flow)                   ← #139, #140
  #142 (Session Integration)             ← #133, #139
  #143 (Polling Service)                 ← #138, #139
  #144 (Harden Input Sanitization)       ← #139

Phase E (Extended):
  #145 (File Exchange)            ← #142
  #146 (Health Monitor)           ← #143
  #147 (Dual-Mode Host)           ← #134, #143

Phase F (Release):
  #148 (Adversarial Tests)        ← #133, #138
  #149 (Release Docs)             ← ALL
```
...
|---|---|---|---|---|---|---|
| 128 | Copilot instructions, custom instructions, and AGENTS.md update | A | ✅ | — | .github/, AGENTS.md | 0 |
| 129 | Architecture documentation (OVERVIEW.md, SECURITY.md update) | A | ✅ | — | docs/ | 0 |
| 130 | Core abstractions (ISessionFactory, ISessionManager, ManagedSession, etc.) | B | ✅ | 128 | Krutaka.Core | ~30 |
| 131 | CorrelationContext agent identity fields | B | ✅ | 128 | Krutaka.Core | ~8 |
| 132 | SessionFactory implementation | B | ✅ | 130 | Krutaka.Tools | ~20 |
| 156 | Logical Session IDs Across Resume/Restore | B | ✅ | 130, 132 | Krutaka.Core, Krutaka.Tools | ~5 |
| 133 | SessionManager with lifecycle management | B | ✅ | 130, 132, 156 | Krutaka.Tools | ~35 |
| 134 | Console Program.cs refactor to ISessionManager | B | ✅ | 133, 156 | Krutaka.Console | ~15 |
| 135 | TelegramSecurityConfig and validation | C | 🟡 | 128 | Krutaka.Core | ~20 |
| 136 | Telegram audit events (default interface methods) | C | 🟡 | 131 | Krutaka.Core | ~15 |
| 137 | Krutaka.Telegram project scaffold | C | 🟡 | 130, 135 | Krutaka.Telegram | ~5 |
| 138 | ITelegramAuthGuard | C | 🟡 | 135, 137 | Krutaka.Telegram | ~30 |
| 139 | ITelegramCommandRouter + basic input sanitizer | D | 🟡 | 138, 137 | Krutaka.Telegram | ~25 |
| 140 | ITelegramResponseStreamer | D | 🟡 | 137 | Krutaka.Telegram | ~20 |
| 141 | Inline keyboard approval flow + HMAC | D | 🟡 | 139, 140 | Krutaka.Telegram | ~25 |
| 142 | Telegram ↔ ISessionManager integration | D | 🟡 | 133, 139 | Krutaka.Telegram | ~15 |
| 143 | Dual-mode polling service | D | 🟡 | 138, 139 | Krutaka.Telegram | ~20 |
| 144 | Harden input sanitization | D | 🟡 | 139 | Krutaka.Telegram | ~15 |
| 145 | File exchange via Telegram | E | 🟡 | 142 | Krutaka.Telegram | ~15 |
| 146 | Health monitoring and push notifications | E | 🟡 | 143 | Krutaka.Telegram | ~10 |
| 147 | Dual-mode host | E | 🟡 | 134, 143 | Krutaka.Console | ~10 |
| 148 | Adversarial tests (isolation + auth + callback + exhaustion) | F | 🟡 | 133, 138 | Tests | ~50 |
| 149 | Release documentation and verification | F | 🟡 | ALL | Docs | 0 |

### Console Migration Path

> **Status:** ✅ Complete (2026-02-15, Issue #134 / PR #160)

The Console application has been refactored from singleton DI to `ISessionManager`. Key implementation details:

1. **Three-step resume pattern**: `ResumeSessionAsync` → `SessionStore.ReconstructMessagesAsync` → `RestoreConversationHistory` (history reconstruction is the caller's responsibility, not `SessionManager`'s, because `Krutaka.Tools` cannot reference `Krutaka.Memory`)
...
2. **Telegram can be disabled**: Remove `Telegram` config section or set empty `AllowedUsers` → bot refuses to start
...
| IAuditLogger extension breaks mocks | High | Medium | Default interface methods with empty bodies — zero mock breakage |
| Sanitization rework between Issue #139 and #144 | Medium | Low | Basic sanitizer in #139, hardening in #144 — clean separation |
...
- `docs/architecture/MULTI-SESSION.md` — Multi-session isolation architecture (created in Issue #129)
- `docs/architecture/TELEGRAM.md` — Telegram security architecture (created in Issue #129)
- `docs/architecture/OVERVIEW.md` — Component architecture (updated in Issue #129)
- `docs/architecture/SECURITY.md` — Security model (updated in Issue #129)
- `docs/architecture/DECISIONS.md` — Architecture Decision Records
- `docs/versions/v0.2.0.md` — v0.2.0 dynamic directory scoping
- `docs/versions/v0.3.0.md` — v0.3.0 graduated command execution
- `docs/guides/TELEGRAM-SETUP.md` — Telegram setup guide (created in Issue #149)
- `CHANGELOG.md` — Version history
- `AGENTS.md` — Agent instructions (updated in Issue #128, verified in Issue #149)
