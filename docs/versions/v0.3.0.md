# Krutaka v0.3.0 — Graduated Command Execution

> **Version:** v0.3.0  
> **Status:** ✅ Complete  
> **Completed:** 2026-02-14  
> **Predecessor:** v0.2.0 (✅ Complete — 903 tests, dynamic directory scoping)  
...
The classification is **static and predictable** — it does NOT change during a session. The agent does NOT "earn" trust. Every command's tier is determined by its executable name and arguments at classification time.
...
System: ⚙ Claude wants to run: git status
        Allow? [Y]es / [N]o
...
1. **Approval fatigue** — Users rubber-stamp every command because most are harmless
2. **Workflow interruption** — Claude chains like `git status` → `dotnet build` → `dotnet test` require 3 manual approvals
3. **Read-only commands have same friction as destructive ones** — `git status` requires the same ceremony as `git push --force`
4. **Diminished security** — When users approve everything habitually, they stop reading prompts — defeating the purpose of human-in-the-loop
...
- ✅ Classify commands into 4 risk tiers based on executable + arguments
- ✅ Auto-approve Safe commands (zero user friction)
- ✅ Auto-approve Moderate commands in trusted directories (synergy with v0.2.0)
- ✅ Always prompt for Elevated commands
- ✅ Always block Dangerous commands (existing behavior, unchanged)
- ✅ Allow users to configure tier overrides for custom toolchains via `appsettings.json`
- ✅ Prevent configuration from promoting Dangerous commands (safety invariant)
- ✅ Show tier information in approval prompts
- ✅ Tell Claude about tiers via system prompt (reduce denied command attempts)
- ✅ Log every command with its tier and approval status

### Non-Goals

- ❌ Dynamic trust progression (agent "earns" trust during a session)
- ❌ Changes to `write_file`/`edit_file` approval logic
- ❌ Changes to directory access policy engine (consumed as-is from v0.2.0)
- ❌ New tool types or tool additions
- ❌ Remote execution (Telegram — that's v0.4.0)
...
RunCommandTool.ExecuteAsync()
  → ISecurityPolicy.ValidateCommand()    [blocklist + metacharacter check]
  → Always requires approval              [IsApprovalRequired("run_command") == true]
  → Execute via CliWrap                   [if approved]
```
...
```text
RunCommandTool.ExecuteAsync()
  → ICommandPolicy.EvaluateAsync()
      → ISecurityPolicy.ValidateCommand()    [blocklist + metacharacter check — ALWAYS FIRST]
      → ICommandRiskClassifier.Classify()     [determine tier]
      → Tier evaluation:
          Safe     → CommandDecision.Approve()             [auto-approved]
          Moderate → Check IAccessPolicyEngine for dir trust
                     → Trusted dir → CommandDecision.Approve()
                     → Untrusted  → CommandDecision.RequireApproval()
          Elevated → CommandDecision.RequireApproval()     [always prompted]
          Dangerous → throw SecurityException              [defense-in-depth]
  → If RequiresApproval → throw CommandApprovalRequiredException → approval flow
  → If Approved → Execute via CliWrap
```
...
│  format, diskpart, powershell, cmd, curl, wget, etc.     │
│  → SecurityException — NEVER executes                     │
│  → Immutable: cannot be changed via config                │
├─────────────────────────────────────────────────────────┤
...
│  → Requires explicit per-invocation approval              │
│  → No "Always" option (same as current run_command)       │
├─────────────────────────────────────────────────────────┤
...
│  mkdir                                                    │
│  → Auto-approved in trusted directories (v0.2.0 synergy)  │
│  → Prompted elsewhere with "Always" option                │
├───��─────────────────────────────────────────────────────┤
│                  SAFE (Auto-Approved)                      │
│  git status/log/diff/show/branch/tag/remote/rev-parse     │
│  dotnet --version/--info/--list-sdks/--list-runtimes       │
│  node/npm/python/pip --version                            │
│  pip list/show/freeze                                     │
│  cat, type, find, dir, where, grep, findstr, tree         │
│  echo, sort, head, tail, wc, diff                         │
│  → Always auto-approved, logged at Debug level             │
└─────────────────────────────────────────────────────────┘
```
...
2. Check BlockedExecutables set → return Dangerous
3. Look up executable in default rules:
   a. If no rules exist for this executable → return Dangerous (fail-closed)
   b. If rules exist:
      i.   Match first argument against argument patterns (case-insensitive)
      ii.  For dotnet: match first TWO arguments (e.g., "nuget push")
      iii. If argument pattern matches → return that rule's tier
      iv.  If no argument pattern matches → return executable's default tier
...
Plus: any executable not found in any tier → Dangerous (fail-closed).
...
GraduatedCommandPolicy.EvaluateAsync(request):
  ...
  ┌──────────────────────────────────────┐
  │ Step 1: Pre-check (SECURITY BOUNDARY) │
  ...
  ┌──────────────▼───────────────────────┐
  │ Step 2: Classify                      │
  │ ICommandRiskClassifier.Classify()     │
  │ Returns CommandRiskTier               │
  └──────────────┬───────────────────────┘
                 │
      ┌───��──────┼──────────┬──────────────┐
      ▼          ▼          ▼              ▼
   ┌──────┐ ┌────────┐ ┌────────┐ ┌──────────┐
   │ Safe │ │Moderate│ │Elevated│ │Dangerous │
   └──┬───┘ └───┬────┘ └───┬────┘ └────┬─────┘
      │         │          │            │
      ▼         ▼          ▼            ▼
...
1. **Cannot promote Dangerous commands** — Any executable in `BlockedExecutables` CANNOT be assigned any other tier via config
2. **Cannot assign Dangerous tier** — Users cannot add to the blocklist via config (code-only change)
3. **No path separators** — Executable must be a simple name
4. **No shell metacharacters** — Neither in executable nor in argument patterns
5. **Validation errors block startup** — Invalid configs throw `InvalidOperationException`
6. **Warnings logged but don't block** — Overly broad rules (null argument patterns) are warned

### Component Diagram

```text
Krutaka.Core (new types):
├── CommandRiskTier              [enum: Safe, Moderate, Elevated, Dangerous]
├── CommandRiskRule               [record: executable + args → tier]
├── CommandExecutionRequest       [record: input to policy]
├── CommandDecision               [record: output from policy]
├── ICommandRiskClassifier        [interface: classify commands]
├── ICommandPolicy                [interface: evaluate command requests]
└── CommandApprovalRequiredException [exception: tier-based approval signal]

Krutaka.Tools (new implementations):
├── CommandRiskClassifier         [implements ICommandRiskClassifier]
├── GraduatedCommandPolicy        [implements ICommandPolicy]
├── CommandPolicyOptions          [options class for config]
└── CommandTierConfigValidator    [startup validation]

Krutaka.Console (modified):
├── ApprovalHandler               [tier-aware display]
└── ServiceExtensions             [register new services]
```
...
4. RunCommandTool calls ICommandPolicy.EvaluateAsync()
   a. GraduatedCommandPolicy calls ISecurityPolicy.ValidateCommand() — pre-check
   b. GraduatedCommandPolicy calls ICommandRiskClassifier.Classify() → Safe
   c. GraduatedCommandPolicy returns CommandDecision(Approved: true, RequiresApproval: false)
5. RunCommandTool sees RequiresApproval == false → executes directly
...
7. User sees: "⚙ Claude wants to run: git push origin main — Risk: 🟡 ELEVATED"
8. User approves → RunCommandTool re-executes → output returned
```
...
|---|---|---|---|---|---|
| T1 | Safe tier bypass | High | Craft arguments so destructive command classifies as Safe | Classification is code-side, not AI-determined. Arguments are pattern-matched against hardcoded rules. Unknown args → higher default tier. | Mitigated |
| T2 | Argument pattern evasion | Critical | Use alias (`-f` for `--force`) to evade classification | Known aliases handled. Unknown aliases → executable's default tier (not Safe). Shell metacharacter check runs FIRST. | Mitigated |
...
1. ✅ **Shell metacharacter check runs FIRST** — Before classification, before everything
2. ✅ **Dangerous commands are ALWAYS blocked** — `powershell`, `cmd`, `format`, etc.
3. ✅ **CliWrap argument arrays** — Never string interpolation for commands
4. ✅ **Environment scrubbing** — Always before child process spawn
5. ✅ **Job Object sandboxing** — Memory/CPU limits on all spawned processes
6. ✅ **CancellationToken on everything** — Timeout enforcement preserved
7. ✅ **Path validation** — Dynamic directory scoping from v0.2.0 preserved
8. ✅ **Audit logging** — Every command logged with correlation IDs
9. ✅ **Untrusted content tagging** — Command output wrapped in XML tags
...
**Mitigation:** Classification matches on first argument only (`push`), not on flags. Both `git push -f` and `git push --force` match the `push` pattern → Elevated. Flags don't change the tier.

#### Vector 2: Compound Command Evasion

**Attack:** `dotnet nuget push` — is this `dotnet` + `nuget` (Elevated) or `dotnet` + unknown?

**Mitigation:** For `dotnet`, classifier checks first TWO arguments. `dotnet nuget` matches the Elevated tier rule for `["nuget"]` first argument. If `dotnet nuget push` — `nuget` matches Elevated, done.
...
> "Session starts → Safe commands only → Agent completes safe tasks → Moderate unlocks"

**Decision: REJECTED.** This was rejected for the following reasons:

| Concern | Explanation |
|---|---|
| Unpredictable state | Security boundary changes during a session — hard to reason about, hard to audit |
...
1. Load appsettings.json → CommandPolicyOptions
2. For each TierOverride:
   a. Is executable in BlockedExecutables? → REJECT (InvalidOperationException)
   b. Is tier set to Dangerous? → REJECT (InvalidOperationException)
   c. Does executable contain path separators? → REJECT
   d. Does executable contain shell metacharacters? → REJECT
   e. Do argument patterns contain shell metacharacters? → REJECT
   f. Is executable empty/null/whitespace? → REJECT
   g. Is argument pattern null (wildcard for all args)? → WARN (log warning, allow)
3. All checks pass → config is valid, application starts
4. Any check fails → application refuses to start with descriptive error
```
...
| Unknown argument | `git worktree` | Moderate (default for git) | Not in any specific pattern → executable's default |
| Flag only | `git -v` | Moderate (default for git) | `-v` not in any pattern → executable's default |
| Compound dotnet | `dotnet nuget push` | Elevated | `nuget` matches Elevated pattern |
| Compound dotnet unknown | `dotnet custom-tool` | Moderate (default for dotnet) | `custom-tool` not in any pattern → dotnet's default |
...
**Safe (auto-approved):**
```text
[dim]⚙ Auto-approved (Safe): git status[/]
```

**Moderate in trusted directory (auto-approved):**
```text
[dim]⚙ Auto-approved (Moderate — trusted dir): dotnet build[/]
```

**Moderate outside trusted directory (prompted):**
```text
⚙ Claude wants to run: dotnet build
  Risk Tier: 🟢 MODERATE (not in trusted directory)
...
**Dangerous (blocked — never shown):**
```text
(Not shown — blocked before reaching UI, error returned to Claude)
```
...
**Dangerous (always blocked):**
  powershell, cmd, curl, wget, and others — these will always fail.
...
|---|---|---|---|
| Safe | Debug | Yes | `[DBG] Command classified: git status → Safe (auto-approved)` |
| Moderate (trusted dir) | Information | Yes | `[INF] Command classified: dotnet build → Moderate (auto-approved, trusted dir: C:\Projects\MyApp)` |
| Moderate (prompted) | Information | No | `[INF] Command classified: dotnet build → Moderate (requires approval)` |
| Elevated | Warning | No | `[WRN] Command classified: git push origin main → Elevated (requires approval)` |
| Dangerous | Error | N/A | `[ERR] Security violation: powershell → Dangerous (blocked)` (existing behavior) |
...
#N+1 (Core abstractions)
  ├──→ #N+2 (Default risk rules + classifier)
  │      └──→ #N+4 (GraduatedCommandPolicy)
  │             ├──→ #N+5 (RunCommandTool refactor)
  │             │      ├──→ #N+6 (Approval UI update)
  │             │      ├──→ #N+7 (System prompt update)
  │             │      └──→ #N+8 (Audit logging)
  │             └──→ #N+9 (Adversarial tests)
  └──→ #N+3 (Config overrides)
         └──→ #N+4 (GraduatedCommandPolicy)
                └──→ #N+10 (Release docs + verification)
```
...
1. **Unit Tests** — Every new class has a corresponding test class
2. **Classification Tests** — Every default tier rule has a dedicated test
3. **Policy Evaluation Tests** — Each tier path tested with mocked dependencies
4. **Configuration Validation Tests** — Every safety invariant has a dedicated test
5. **Adversarial Tests** — Attack vector simulations following existing pattern
6. **Integration Tests** — `RunCommandTool` with real classifier and policy
7. **Regression Tests** — All existing 903+ tests must continue to pass
...
1. **Revert to v0.2.0 behavior:** Set all tiers to Elevated in config → everything requires approval (same as v0.2.0)
2. **Disable classifier:** Remove `ICommandPolicy` registration from DI → falls back to `ISecurityPolicy.IsApprovalRequired()` which returns `true` for `run_command`
...
| Approval fatigue reduction leads to less attention on Elevated prompts | Low | Medium | Elevated prompts shown with 🟡 warning, audit logging captures all decisions |
...
- `docs/versions/v0.2.0.md` — Predecessor version (dynamic directory scoping)
- `docs/architecture/OVERVIEW.md` — Component architecture
- `docs/architecture/SECURITY.md` — Security threat model
...
- `docs/guides/APPROVAL-HANDLER.md` — Approval handler documentation
- `src/Krutaka.Tools/CommandPolicy.cs` — Current allowlist/blocklist implementation
- `src/Krutaka.Core/ISecurityPolicy.cs` — Current security policy interface
- `src/Krutaka.Tools/RunCommandTool.cs` — Current command execution tool
- `src/Krutaka.Tools/ServiceExtensions.cs` — Current DI registration
