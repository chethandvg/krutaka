# Krutaka v0.4.5 — Session Resilience, API Hardening & Context Intelligence
...
This release does NOT introduce new user-facing features — it makes the existing features dramatically more reliable.

## Motivation — Real-World Failures
...
6. No try-catch around `CompactIfNeededAsync()` → unhandled exception crashed the main loop
...
- Claude tried relative paths, absolute paths, and various guesses — all failed
- User eventually had to manually copy the code output

### Failure 3: Rate Limit (Identified in Outline_gaps.md)

Anthropic's rate limits (RPM, ITPM, OTPM) return 429 with `retry-after` header. The current `ClaudeClientWrapper` has zero retry logic for rate limits — a single 429 crashes the agentic loop.
...
Large tool results from older turns (e.g., a 50KB file read 10 turns ago) remain in the conversation history at full size, consuming tokens unnecessarily. OpenClaw trims these to `[Previous tool result truncated — 51,200 chars]` after N turns.
...
- **Add error recovery**: Graceful handling of `AnthropicBadRequestException` in main loop — offer session repair or `/new`
...
- Interactive working directory prompt (deferred — requires Program.cs startup restructuring)
- Telegram setup wizard (deferred — requires new setup infrastructure)
- Vector search or Memory v2 (deferred per ADR-009)
- Changes to command risk classification (v0.3.0 — consumed as-is)

## Immutable Security Boundaries (NEVER change)

All v0.1.0–v0.4.0 boundaries remain. No new boundaries introduced.
- Session repair MUST NOT fabricate tool results that could influence Claude's behavior — synthetic results MUST be marked `is_error = true`
- Tool result pruning MUST NOT modify persisted JSONL — only in-memory messages sent to Claude API
...
SessionStore.ReconstructMessagesAsync()
  → ParseToolInput() ensures JsonElement (not string) for tool_use.input
  → RepairOrphanedToolUseBlocks() detects orphaned IDs via set difference
  → NEW: PostRepairValidation() re-scans to verify invariant holds
  → Returns validated message list

AgentOrchestrator.CompactIfNeededAsync()
  → NEW: try-catch wrapper — on failure, logs error and skips compaction
  → Never crashes the agentic loop
```
...
```
ClaudeClientWrapper
  → SendMessageAsync(): catch AnthropicRateLimitException → retry with backoff
  → CountTokensAsync(): catch AnthropicRateLimitException → retry with backoff
  → Backoff strategy: 1s, 2s, 4s, 8s (max 30s, max 3 retries)
...
SystemPromptBuilder.BuildAsync()
  → Layer 3c (NEW): Environment Context
  → Includes: DefaultWorkingDirectory, CeilingDirectory, AutoGrantPatterns
  → Injected via ToolOptions (already available in DI)
  → Claude knows exactly where it can write files
```
...
ContextCompactor.CompactAsync()
  → NEW: Before summarization, generate a "memory flush" prompt
  → Asks Claude to extract: key decisions, file paths, user preferences, progress
  → Writes extracted context to MEMORY.md via MemoryFileService
  → Then proceeds with normal compaction
  → Console: brief "📝 Saving context..." indicator
  → Telegram: silent (no user message)
```
...
AgentOrchestrator.RunAgenticLoopAsync()
  → Before each SendMessageAsync() call
  → NEW: PruneOldToolResults(conversationSnapshot)
  → For tool_result blocks older than N turns (configurable, default: 6):
    → If content length > threshold (default: 1000 chars):
      → Replace with: "[Previous tool result truncated — {X} chars. Use read_file to re-read if needed.]"
  → Does NOT modify _conversationHistory (only the snapshot sent to Claude)
  → Does NOT modify JSONL (permanent history intact)
```
...
SystemPromptBuilder.LoadCoreIdentityAsync()
  → NEW: Cap at MaxBootstrapCharsPerFile (default: 20,000)
  → NEW: Total cap across all files: MaxBootstrapTotalChars (default: 150,000)
  → Truncation marker: "[... truncated at 20,000 chars. Use read_file for full content ...]"
  → Layer 2 (Security Instructions) is hardcoded — NEVER truncated
```
...
```
SessionStore
  → NEW event type: "compaction"
  → Written after CompactAsync() completes
  → Contains: summary, tokensBefore, tokensAfter, messagesRemoved, timestamp
  → LoadAsync() skips compaction events during message reconstruction
  → Useful for debugging session issues in both Console and Telegram
```
...
- **Mitigation:** Synthetic results MUST always have `is_error = true` and content = "Session was interrupted before tool execution completed". Already enforced in current code — verify in tests.
...
- **Mitigation:** Only prune results older than N turns (default: 6). Error results (is_error=true) are pruned with message: "[Previous tool error truncated — {X} chars. Original error occurred {N} turns ago.]" — preserves error awareness.
...
- **Mitigation:** Layer 2 (Security Instructions) is hardcoded in `GetSecurityInstructions()` — it is NEVER loaded from files and cannot be truncated. Only Layer 1 (core identity from AGENTS.md) and Layer 5 (MEMORY.md) are subject to caps.
...
- **Mitigation:** Compaction events are informational only — they don't affect message reconstruction. `ReconstructMessagesAsync()` skips them entirely.
...
| # | Title | Phase | Type |
|---|-------|-------|------|
| #178 | Update Copilot instructions and AGENTS.md for v0.4.5 | A: Docs | Documentation |
| #179 | Fix orphaned tool_use session resume crash | B: Bugs | Bug Fix |
| #180 | Add retry/backoff for Anthropic API rate limits | B: Bugs | Bug Fix |
| #181 | Add error recovery in main loop for API exceptions | B: Bugs | Bug Fix |
| #182 | Add directory awareness to system prompt | C: Prompt | Enhancement |
| #183 | Add bootstrap file size caps to system prompt | C: Prompt | Enhancement |
| #184 | Add pre-compaction memory flush to MEMORY.md | D: Context | Enhancement |
| #185 | Add tool result pruning for older conversation turns | D: Context | Enhancement |
| #186 | Add compaction events to JSONL session files | D: Context | Enhancement |
| #187 | Adversarial tests for session resilience and API hardening | E: Test | Testing |
| #188 | v0.4.5 release documentation and verification | E: Release | Documentation |

## Acceptance Criteria (Epic Level)

- [ ] All existing 1,765 tests continue to pass (2 skipped — zero new failures)
- [ ] New tests bring total to ~1,870+
- [ ] Console mode: Resuming a session with orphaned tool_use blocks no longer crashes
- [ ] Console mode: Rate limit (429) during agentic loop retries with backoff, does not crash
- [ ] Console mode: `AnthropicBadRequestException` in main loop offers recovery, does not crash
- [ ] Claude knows the working directory, ceiling directory, and auto-grant patterns
- [ ] Large bootstrap files truncated with marker, security instructions never truncated
- [ ] Pre-compaction flush writes key context to MEMORY.md before summarization
- [ ] Tool results older than 6 turns are pruned in API calls (not in JSONL)
- [ ] Compaction events recorded in JSONL for debugging
- [ ] Telegram mode backward-compatible (same improvements apply)
- [ ] All documentation updated to reflect v0.4.5 changes
