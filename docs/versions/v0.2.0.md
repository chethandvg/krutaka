...
> **Milestone:** [v0.2.0](https://github.com/chethandvg/krutaka/milestone/1)
...
```
Session starts
  → Agent requests access to directory (tool invocation triggers it)
  → LayeredAccessPolicyEngine evaluates request through 4 layers
  → Grant/Deny with scoped path and optional TTL
  → Tool proceeds with validated ScopedPath
...
sequenceDiagram
    ...
    UI-->>Tool: Approved
    Tool->>Engine: EvaluateAsync(path, ReadOnly, "reading config")
    Note over Engine,L3: Second call — Layer 3 finds session grant
...
|-----------|---------------|-------------|--------|------|------------|
| File read | PathResolver → Engine eval | File.ReadAllTextAsync | ~1ms | Low | Re-resolve at access |
| File write | PathResolver → Engine eval → Approval | File.WriteAllTextAsync | ~100ms (human approval) | Medium | Re-resolve after approval, before write |
| Command exec | PathResolver → Engine eval → Approval | CliWrap.ExecuteAsync | ~100ms | Medium | Re-validate working directory, Job Object limits damage |

### Symlink and Junction Security

| Scenario | PathResolver Behavior | Result |
|----------|----------------------|--------|
| Regular file in allowed dir | Returns same path | ✅ Allowed |
| Symlink in allowed dir → allowed target | Returns resolved target | ✅ Allowed |
| Symlink in allowed dir → blocked target | Returns resolved target | ❌ Layer 1 deny |
| Junction in allowed dir → blocked target | Returns resolved target | ❌ Layer 1 deny |
| Circular symlink (A → B → A) | Detects cycle, returns error | ❌ Rejected |
| Non-existent file in allowed dir | Validates parent chain | ✅ Allowed (if parent is valid) |
| Non-existent file, parent is symlink to blocked | Resolves parent → blocked | ❌ Layer 1 deny |

### Glob Pattern Abuse Prevention

| Pattern | Validation Result | Reason |
|---------|-------------------|--------|
| `C:\Users\chethan\Projects\**` | ✅ Valid | 4 segments, specific path |
| `C:\Users\chethan\**` | ✅ Valid (with warning) | 3 segments, borderline broad |
| `C:\Users\**` | ❌ Rejected | 2 segments, too broad |
| `C:\**` | ❌ Rejected | 1 segment, catastrophically broad |
| `**` | ❌ Rejected | No root, matches everything |
| `C:\Users\chethan\AppData\**` | ❌ Rejected | Contains blocked directory |
| `..\..\..\**` | ❌ Rejected | Relative traversal, no anchor |
| `D:\Backup\**` | ❌ Rejected (if ceiling is C:) | Outside ceiling directory |
...
- [ ] Symlink to blocked dir → denied (verified with adversarial test)
