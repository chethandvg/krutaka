# Krutaka v0.5.0 — Autonomous Agent Mode
...
1. **Autonomy levels** — Graduated approval control (Supervised → Guided → Semi-Autonomous → Autonomous)
2. **Task budgets** — Hard caps on tokens, tool calls, file modifications, and processes per session
3. **Git checkpoints** — Automatic pre-modification snapshots with rollback capability
4. **Deadman's switch** — Auto-pause and auto-abort when the user is unresponsive
5. **Behavior anomaly detection** — Real-time monitoring for runaway loops, scope creep, and escalation attempts

**Default behavior is unchanged.** Level 1 (Guided) is the default — same as v0.3.0+. Users must explicitly opt in to higher autonomy.
...
In v0.4.x, every Moderate and Elevated tool invocation requires human approval. For a 30-minute coding task, this can mean **50+ approval prompts**. Users rubber-stamp approvals, defeating the security purpose. The agent also cannot work unattended — closing the console or Telegram stops all progress.

### Real-World Impact

```text
User: "Refactor the Calculator class to use dependency injection"
Claude: calls read_file("Calculator.cs")          → auto-approved ✅
Claude: calls write_file("ICalculator.cs", ...)    → APPROVAL NEEDED ❌ (user clicks Yes)
Claude: calls write_file("Calculator.cs", ...)     → APPROVAL NEEDED ❌ (user clicks Yes)
Claude: calls edit_file("Program.cs", ...)         → APPROVAL NEEDED ❌ (user clicks Yes)
Claude: calls run_command("dotnet", ["build"])      → APPROVAL NEEDED ❌ (user clicks Yes)
Claude: calls edit_file("Calculator.cs", ...)      → APPROVAL NEEDED ❌ (user clicks Yes)
Claude: calls run_command("dotnet", ["test"])       → APPROVAL NEEDED ❌ (user clicks Yes)
...
```text
User: "Refactor the Calculator class to use dependency injection"
Claude: calls read_file("Calculator.cs")          → auto-approved ✅ (Safe)
Claude: calls write_file("ICalculator.cs", ...)    → auto-approved ✅ (Moderate, Level 2)
Claude: calls write_file("Calculator.cs", ...)     → auto-approved ✅ (Moderate, Level 2)
Claude: calls edit_file("Program.cs", ...)         → auto-approved ✅ (Moderate, Level 2)
Claude: calls run_command("dotnet", ["build"])      → auto-approved ✅ (Moderate, Level 2)
Claude: calls run_command("dotnet", ["test"])       → auto-approved ✅ (Moderate, Level 2)
Claude: calls run_command("git", ["push"])          → APPROVAL NEEDED ❌ (Elevated, always prompted)
```
...
- G9: Zero regressions — all 2,051 existing tests pass unchanged
...
2. Validate: if Level 3 and Agent:AllowAutonomousMode != true → reject, fallback to Level 2
...
- Agent CANNOT call rollback — only `/rollback` command triggers it.
...
While agent is working on a task:
  /steer "change direction to X"  → Injects steering message into conversation
  /pause                          → Agent completes current tool call, then waits
  /continue                       → Resumes from Paused state
  /abort                          → Stops immediately, rollback offered
  /budget                         → Shows current budget consumption
  /checkpoint                     → Creates manual checkpoint
  /rollback [latest|<id>]         → Rolls back to checkpoint
  /autonomy                       → Shows current autonomy level
```
...
AgentOrchestrator (modified)
  │
  ├── AutonomyLevel (from config + per-session, immutable after session start)
  ├── ITaskBudgetTracker (per-session, created by SessionFactory)
  ├── IGitCheckpointService (per-session, created by SessionFactory)
  ├── IBehaviorAnomalyDetector (per-session, created by SessionFactory)
  └── AgentState machine (Running ↔ Paused → Aborted)

SessionManager (modified)
  │
  └── DeadmanSwitch timer (per-session, in SessionManager scope)

Before each tool call:
  1. Check budget → reject if exhausted (yield BudgetExhausted)
  2. Check autonomy level → auto-approve or prompt based on tier vs level
  3. Check anomaly detector → pause if anomalous (yield AnomalyDetected)
...
|---|--------|----------|---------------|------------|---------------|
| AT1 | Autonomy level escalation | 🔴 High | Prompt injection → agent modifies config | Level stored in config, confirmed at session start. No runtime escalation API. `/autonomy` is read-only. | Low |
| AT2 | Budget exhaustion via prompt injection | 🟡 Medium | Crafted file content → "call these 100 tools" | Hard budget cap. Per-session. Cannot be increased by agent. | Low |
| AT3 | Budget bypass via token manipulation | 🟡 Medium | Agent generates large tool results counted differently | Server-side token count authoritative. Budget tracks ALL usage. | Low |
| AT4 | Checkpoint manipulation (disk fill) | 🟢 Low | Agent creates excessive checkpoints | Max 50 checkpoints per session. Old checkpoints auto-pruned. | Very Low |
| AT5 | Rollback to inject malicious state | 🟡 Medium | Agent rolls back to a checkpoint with staged malicious files | Rollback is user-initiated only. Agent CANNOT call rollback. | Low |
| AT6 | Anomaly detector bypass (threshold gaming) | 🟡 Medium | Agent keeps behavior just below thresholds | Sliding window + combined metric scoring. Configurable thresholds. | Medium |
| AT7 | Deadman's switch circumvention | 🟢 Low | Agent sends synthetic "heartbeat" | Timer in SessionManager. Only genuine user input resets. | Very Low |
| AT8 | Autonomous + Telegram = unattended | 🔴 High | Remote Level 3, user walks away | Config opt-in + session confirmation + budget cap + deadman auto-abort. Telegram has stricter defaults. | Low |
| AT9 | Steering command injection | 🟡 Medium | `/steer` content contains prompt injection | Wrapped in `<untrusted_content>` tags. Telegram: 7-layer sanitization. | Low |
| AT10 | Session command abuse | 🟢 Low | Rapid `/pause`/`/continue` cycling | Debounce. Minimum 1s between state changes. State machine with clear transitions. | Very Low |
| AT11 | Git operations in non-git directories | 🟡 Medium | Checkpoint service fails in non-git dir | Graceful degradation. Checkpoints are optional — file mods still work. | Low |
| AT12 | Concurrent budget race conditions | 🟡 Medium | Race between check and consume | Interlocked atomic operations. Check-then-consume is atomic. | Very Low |
...
- **S9: Autonomy level NEVER escalates at runtime** — only set at session start from config
- **S10: Budget NEVER increases without explicit user action** — agent cannot extend its own budget
...
- **S14: Dangerous tier commands are ALWAYS blocked** — regardless of autonomy level (existing S1 reinforced)
- **S15: All steering input wrapped in `<untrusted_content>`** — no exception
...
Expected behavior:
  - Agent reads the file → wraps in <untrusted_content>
  ...
  ✅ Attack fails — no escalation possible
```
...
Expected behavior:
  ...
  - At tool call 80/100: BudgetWarning event → user notified
  - At tool call 100/100: BudgetExhausted event → agent loop stops
  - User prompted: "Budget exhausted. /extend to add more or /abort"
  ✅ Attack limited — hard cap enforced
```
...
Expected behavior:
  ...
  ✅ Stuck loops detected and interrupted
```
...
All interfaces and models defined as stubs in v0.4.6 — ready for implementation:

| Type | Location | Tests |
|------|----------|-------|
| `AutonomyLevel` enum | `src/Krutaka.Core/Models/AutonomyLevel.cs` | ✅ 3 tests |
| `TaskBudget` record | `src/Krutaka.Core/Models/TaskBudget.cs` | ✅ Tests |
| `BudgetDimension` enum | `src/Krutaka.Core/Models/BudgetDimension.cs` | ✅ Tests |
| `TaskBudgetSnapshot` record | `src/Krutaka.Core/Models/TaskBudgetSnapshot.cs` | ✅ Tests |
| `ITaskBudgetTracker` interface | `src/Krutaka.Core/Abstractions/ITaskBudgetTracker.cs` | ✅ Stub tests |
| `CheckpointInfo` record | `src/Krutaka.Core/Models/CheckpointInfo.cs` | ✅ Tests |
| `IGitCheckpointService` interface | `src/Krutaka.Core/Abstractions/IGitCheckpointService.cs` | ✅ Stub tests |
| `AnomalySeverity` enum | `src/Krutaka.Core/Models/AnomalySeverity.cs` | ✅ Tests |
| `AnomalyAssessment` record | `src/Krutaka.Core/Models/AnomalyAssessment.cs` | ✅ Tests |
| `AgentBehaviorSnapshot` record | `src/Krutaka.Core/Models/AgentBehaviorSnapshot.cs` | ✅ Tests |
| `IBehaviorAnomalyDetector` interface | `src/Krutaka.Core/Abstractions/IBehaviorAnomalyDetector.cs` | ✅ Stub tests |
...
Phase A (no dependencies):
  ...
  #244 (Agent State Machine)                    ← #243

Phase B (Autonomy):
  #245 (AutonomyLevelProvider + Integration)    ← #244
  #246 (/autonomy Command)                      ← #245

Phase C (Budget):
  #247 (TaskBudgetTracker + Integration)        ← #244
  #248 (/budget Command)                        ← #247

Phase D (Checkpoints):
  #249 (GitCheckpointService + Integration)     ← #247
  #250 (/rollback + /checkpoint Commands)       ← #249

Phase E (Deadman's Switch):
  #251 (DeadmanSwitch + Integration)            ← #244
  #252 (DeadmanSwitch Notifications)            ← #251

Phase F (Anomaly Detection):
  #253 (BehaviorAnomalyDetector + Integration)  ← #247
  #254 (AnomalyDetection Configuration)         ← #253

Phase G (Steering):
  #255 (/steer + /pause + /continue)            ← #244, #251
  #256 (/abort + Rollback Integration)          ← #255, #250

Phase H (UI Integration):
  #257 (Console UI v0.5.0 Features)             ← #256, #248, #254
  #258 (Telegram UI v0.5.0 Features)            ← #257

Phase I (Release):
  #259 (Autonomy + Budget Adversarial Tests)    ← #246, #248
  #260 (Checkpoint + Anomaly Adversarial Tests) ← #250, #254, #252
  #261 (v0.5.0 Release Documentation + Verification) ← ALL
```
...
|---|-------|-------|------|-----------|
| 242 | v0.5.0 Autonomous Agent Mode — EPIC | — | Epic | All below |
| 243 | v0.5.0 Custom Instructions, AGENTS.md, and Documentation Updates | A | Docs | — |
...
4. No existing interfaces modified — only new optional parameters added
...
### Pros
1. **Eliminates approval fatigue** — 50+ prompts → 2-5 at Level 2
2. **Budget safety net** — Hard caps prevent runaway token consumption
3. **Git rollback** — File modifications are reversible
4. **Anomaly detection** — Catches stuck agents and potential attacks
5. **Backward compatible** — Default Level 1 = same behavior as v0.3.0+
6. **Per-session isolation** — All state is per-session (v0.4.0 architecture)
7. **Deadman's switch** — Auto-abort prevents resource waste

### Cons
1. **Complexity** — 5 new subsystems, orchestrator grows significantly
2. **Git dependency** — Checkpoints only work in git repos
3. **False positives** — Anomaly detection may interrupt legitimate work
4. **Budget guessing** — Users may not know how many tokens a task needs
5. **Testing burden** — ~350 new tests, many involving async/timing
...
- `docs/versions/v0.4.6.md` — v0.4.6 prerequisites and interfaces
- `docs/architecture/OVERVIEW.md` — Component architecture
- `docs/architecture/SECURITY.md` — Security model
- `docs/architecture/MULTI-SESSION.md` — Multi-session isolation
